...
function getCachedTodayFortune(todayStr) {
  const cachedDate = get(KEYS.TODAY_FORTUNE_DATE)
  if (cachedDate === todayStr) {
    return get(KEYS.TODAY_FORTUNE, null)
  }
  return null
}
...
function appendFortuneHistory(record) {
  if (!record || !record.is_official) return
  const list = getLocalHistory()
  const key = record.fortune_date
  const filtered = list.filter((item) => item.fortune_date !== key)
  filtered.unshift(record)
  set(KEYS.FORTUNE_HISTORY, filtered.slice(0, HISTORY_MAX))
}

function mergeHistoryRecords(cloudList, localList) {
  const map = {}
