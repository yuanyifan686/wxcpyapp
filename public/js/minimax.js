...
export function rollLevel() {
  ...
  for (const item of LEVEL_WEIGHTS) {
    acc += item.weight
    if (r < acc) return item.level
  }
  return 'N'
}

export function rollScoreForLevel(level) {
...
function parseFortuneJson(text, preset) {
  ...
  return normalizeFortune(JSON.parse(raw), preset)
}
...
export function normalizeFortune(data, preset) {
  const buff = data.buff || {}
  const level = preset?.level || data.level || 'R'
  const score = preset
    ? preset.score
...
function mockFortune(preset) {
  const level = preset?.level || rollLevel()
  const score = preset?.score || rollScoreForLevel(level)
  const titles = {
...
export async function generateFortune(useMock = false) {
  ...
  if (useMock) return mockFortune(preset)
  ...
  for (let left = RETRY_COUNT; left >= 0; left--) {
    try {
      ...
      return parseFortuneJson(content, preset)
    } catch (err) {
      if (left === 0) {
